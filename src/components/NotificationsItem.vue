<template>
  <li class="w-1/2 relative flex justify-center items-center px-4 py-2 mb-2 font-bold" :class="style(item.type)">
        <svg v-if="item.type === 'success'" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="fill-current">
          <circle cx="15.5" cy="9.5" r="1.5"/>
          <circle cx="8.5" cy="9.5" r="1.5"/>
          <path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm0-2.5c2.33 0 4.32-1.45 5.12-3.5h-1.67c-.69 1.19-1.97 2-3.45 2s-2.75-.81-3.45-2H6.88c.8 2.05 2.79 3.5 5.12 3.5z"/>
        </svg>
        <svg v-if="item.type === 'error'" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="fill-current">
          <circle cx="15.5" cy="9.5" r="1.5"/>
          <circle cx="8.5" cy="9.5" r="1.5"/>
          <path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm0-6c-2.33 0-4.32 1.45-5.12 3.5h1.67c.69-1.19 1.97-2 3.45-2s2.75.81 3.45 2h1.67c-.8-2.05-2.79-3.5-5.12-3.5z"/>
        </svg>
        <div class="flex-1 text-center">{{item.text}}</div>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="fill-current cursor-pointer" @click="close">
          <path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>
        </svg>
        <div class="w-full absolute pin-b h-1 bg-woodsmoke notification-timer"></div>
      </li>
</template>

<script>
export default {
  name: "notifications-item",
  props: {
    item: {
      type: Object,
      required: true
    }
  },
  methods: {
    style(type) {
      if (type === 'success') {
        return 'bg-malachite';
      }

      return 'bg-cinnabar';
    },
    close() {
      clearTimeout(this.timer);

      this.$emit('remove', this.item);
    }
  },
  mounted() {
    this.timer = setTimeout(this.close, 5000);
  }
}
</script>

<style>
  .notification-timer {
    transform-origin: top left;
    animation: notification-timer 5s linear;
  }

  @keyframes notification-timer {
    0% {
      transform: scaleX(1);
    }

    100% {
      transform: scaleX(0);
    }
  }
</style>
